...
use super::AppState;
use crate::models::{PlayerDetail, PlayerListItem};
...
async fn get_players(State(state): State<AppState>) -> Json<Vec<PlayerListItem>> {
...
async fn get_player_sessions(
    ...
    let server_name_map: HashMap<i32, String> =
    ...
    for (start, end, server_id) in &merged {
        ...
        let server_group = server_name_map
            .get(server_id)
            .cloned()
        ...
        while current < *end {
            ...
            let _next_day = (current + Duration::days(1))
                .date_naive()
                .and_hms_opt(0, 0, 0)
                .unwrap();
            let next_day_utc = chrono::NaiveDateTime::new(
                current.date_naive(),
                chrono::NaiveTime::from_hms_opt(0, 0, 0).unwrap(),
            ) + Duration::days(1);
            let seg_end = if end.naive_utc() < next_day_utc {
                end.naive_utc()
            } else {
                next_day_utc
            };
            ...
            entry.sessions.push(SessionInfo {
                ...
                end: format_gmt8_naive(std::cmp::min(
                    *end,
                    chrono::DateTime::<Utc>::from_naive_utc_and_offset(seg_end, Utc),
                )),
    ...
    let days_count = if dates_sorted.is_empty() {
        1.0
    } else {
        dates_sorted.len() as f64
    };
    ...
    let daily_result: Vec<serde_json::Value> = dates_sorted
        .iter()
        .map(|day| {
            let entry = &daily[day];
            serde_json::json!({
    ...
    let hourly_average: Vec<serde_json::Value> = (0..24)
        .map(|hour| {
            serde_json::json!({
                "hour": hour,
    ...
    let avg_session = if session_count > 0 {
        total_duration / session_count as f64
    } else {
        0.0
    };
...
async fn get_player_weekly_stats(
    ...
    let weekday_preference: Vec<serde_json::Value> = (0..7)
        .map(|d| {
            let day_count = weekday_totals[d].days.len() as f64;
            let avg = if day_count > 0.0 {
                weekday_totals[d].total / day_count
            } else {
                0.0
            };
            serde_json::json!({
...
async fn get_player_heatmap(
    ...
    match state.db.get_player_heatmap(&name, days).await {
        Ok(heatmap) => Json(serde_json::to_value(heatmap).unwrap_or(serde_json::json!([]))),
...
fn merge_intervals(
    mut intervals: Vec<(DateTime<Utc>, DateTime<Utc>, i32)>,
) -> Vec<(DateTime<Utc>, DateTime<Utc>, i32)> {
...
fn merge_intervals_opt(
    mut intervals: Vec<(DateTime<Utc>, DateTime<Utc>, Option<i32>)>,
) -> Vec<(DateTime<Utc>, DateTime<Utc>, Option<i32>)> {
    if intervals.is_empty() {
        return vec![];
    }
    intervals.sort_by(|a, b| a.0.cmp(&b.0));
