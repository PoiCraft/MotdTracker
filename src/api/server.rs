...
use super::AppState;
use crate::models::{LatencyStats, NodeStatus, NodeWithStats, ServerHead, StatusLog};
...
/// 获取所有节点及统计
async fn get_nodes(State(state): State<AppState>) -> Json<Vec<NodeWithStats>> {
    ...
    let result: Vec<NodeWithStats> = servers
        ...
        .map(|server| {
            ...
            let latency_stats = history.get(&server.id).map(|h| calculate_latency_stats(h));
...
/// 获取所有节点聚合状态
async fn get_server_head(State(state): State<AppState>) -> Json<ServerHead> {
    let servers = match state.db.get_all_servers().await {
        Ok(s) => s,
        Err(_) => {
            return Json(ServerHead {
                total_nodes: 0,
                online_nodes: 0,
    ...
    let total_players_online: u32 = latest_status
        .iter()
        .filter_map(|s| s.players_online.map(|n| n as u32))
        .sum();
    let total_players_max: u32 = latest_status
        .iter()
        .filter_map(|s| s.players_max.map(|n| n as u32))
        .sum();

    let latencies: Vec<f64> = latest_status.iter().filter_map(|s| s.latency).collect();
...
/// 获取 24h 统计
async fn get_server_stats(State(state): State<AppState>) -> Json<HashMap<i32, LatencyStats>> {
    let history = match state.db.get_all_history(24).await {
        Ok(h) => h,
...
/// 获取状态时间线
async fn get_status_timeline(State(_state): State<AppState>) -> Json<serde_json::Value> {
    Json(serde_json::json!({ "message": "Not implemented yet" }))
}

/// 获取在线玩家
async fn get_server_players(State(state): State<AppState>) -> Json<serde_json::Value> {
...
/// 获取服务器配置
async fn get_server_config(State(state): State<AppState>) -> Json<serde_json::Value> {
